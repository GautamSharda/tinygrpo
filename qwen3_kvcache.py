...
def qwen3_tg(prompt: str, max_new_tokens=5):
    ...
    def _transformer(E: tinygrad.Tensor, i, kv_cache=None) -> tinygrad.Tensor:
        ...
        def _flash_gqa(Q, K, V, mask: tinygrad.Tensor | None = None):
            B, Hq, Lq, d = Q.shape           # (1, 16, 7, 128)
            _, Hk, Lk, _ = K.shape           # (1, 8, 7, 128)
            n_rep = Hq // Hk                # 2
            # K: (1, 8, 7, 128) -> (1, 16, 7, 128)
            K = K.unsqueeze(2).expand(B, Hk, n_rep, Lk, d).reshape(B, Hk * n_rep, Lk, d)
            V = V.unsqueeze(2).expand(B, Hk, n_rep, Lk, d).reshape(B, Hk * n_rep, Lk, d)
        ...
        if kv_cache is not None:
            ...
            if i in kv_cache:
                cached_K = kv_cache[i]['K']
                cached_V = kv_cache[i]['V']
                cached_seq_len = cached_K.shape[2]
                
                if current_seq_len > cached_seq_len:
                    # We have new tokens to process
                    # Use cached K,V for the previous tokens and compute only for new tokens
                    new_K = K[:, :, cached_seq_len:, :]
                    new_V = V[:, :, cached_seq_len:, :]
                    K = cached_K.cat(new_K, dim=2)
                    V = cached_V.cat(new_V, dim=2)
                    kv_cache[i] = {'K': K, 'V': V}
                else:
                    # Use cached values if sequence length hasn't increased
                    K = cached_K
                    V = cached_V
            else:
                # First time: store full K,V
                kv_cache[i] = {'K': K, 'V': V}
        
        # 9.  Grouped‑Query Attention ----------------------------------------
        att = _flash_gqa(Q, K, V)    # (B,L,16,128)
        # 10.  concat heads & output proj -------------------------------------
    ...
    for _ in range(max_new_tokens):
        # Always process the full sequence but use cached K,V when available
        act = embeddings
        
        for l in range(28):
            act = _transformer(act, l, kv_cache)
